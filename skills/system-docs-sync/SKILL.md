...
시스템 운영 문서를 **로컬(~/.claude/) + Notion** 양쪽에 동시 반영하는 동기화 스킬.
한 곳 수정하면 → 나머지도 자동으로 맞춰주고 → 연결된 파일까지 연쇄 수정한다.

> **비유**: 주방 레시피북이 2권 있다고 생각하세요.
> 하나는 주방 벽에 붙어있고(로컬), 하나는 사무실 캐비넷에 있다(Notion).
> 이 스킬은 한 권 수정하면 다른 한 권도 알아서 고쳐주는 비서입니다.
> 게다가 "소스 레시피를 바꿨으니 파스타 레시피에서 참조하는 부분도 같이 고쳐야지" 까지 해줍니다.
...
| 8 | *(통합본 — 로컬 파일 없음)* | `3317f0809621816688feef408023224b` | **Claude 운영 지침 v4.0 통합본** (Claude.ai가 읽는 원본) |

> **중요**: 8번 통합본은 개별 파일(1~7)의 CLAUDE.md 해당 섹션을 합친 마스터 문서.
> 개별 페이지만 수정하고 통합본을 빠뜨리면 → Claude.ai가 변경 사항을 인지하지 못함.
> **반드시 개별 페이지 + 통합본 양쪽 모두 동기화할 것.**
...
**동기화 방향:**
- 개별 파일(CLAUDE.md 등) 수정 시 → 운영 지침 해당 섹션도 갱신
- 운영 지침 직접 수정 시 → 해당하는 개별 파일도 갱신
...
3. **운영 지침이 마스터**: 개별 파일과 운영 지침이 충돌하면 → 운영 지침이 우선, 개별 파일을 운영 지침에 맞춤
4. **Notion 저장 규칙 준수**: 이 스킬은 "동기화"이므로 별도 확인 없이 바로 반영 (신규 저장이 아님)
5. **대규모 구조 변경 시**: 예시 1개 먼저 보여주고 승인 후 나머지 (핵심원칙 5번 준수)

---

## Claude.ai에서 이 스킬이 트리거될 때

Claude.ai 환경에서는 로컬 파일에 직접 접근할 수 없다.
이 경우 다음 절차를 따른다:

1. **Notion 페이지는 직접 수정** (Notion MCP 사용)
2. **로컬 파일 수정은 지시서 생성** → `~/.claude/` 파일 수정 내용을 담은 지시서를 작성
3. 지시서를 대표님께 전달 → Claude Code에서 실행하거나 수동 반영
4. 또는 대표님이 Claude Code 세션을 열면 → Claude Code가 Notion을 읽고 로컬 파일을 자동 맞춤

**지시서 형식:**
```
# system-docs-sync 로컬 반영 지시서
날짜: YYYY-MM-DD
대상: ~/.claude/CLAUDE.md
변경: [수정 내용 상세]
Notion 반영: 완료 ✅
로컬 반영: 미완 → Claude Code에서 실행 필요
```

---

*system-docs-sync v3.0 | 2026.04.07 | v4.0 반영 — 통합본 동기화 대상 추가*
