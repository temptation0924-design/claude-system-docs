...
### 📋 Wave 2 출력 공통 규칙 (v2.0 — 3단 분류 강제)

각 오류 JSON의 `errors[]` 레코드에 **반드시 `category` 필드** 포함:
- `"category": "A"` — 입력 오류 (DB 정합성 문제, 즉시 수정)
- `"category": "B"` — 확인사항 (담당자 액션 필요)
- `"category": "C"` — 진행 추적 (일상 워크플로우)

각 규칙 파일(rules/*.md) 분기에 category 태그가 있으면 그대로 사용. 없으면 다음 기본 규칙:
- 누락/미입력/미연결/유령링크/빈값 → A
- 체크박스/토글/독촉/완납대기/긴급대처 → B
- 상습/기간내/문자/대기/진행중 → C

슬랙 섹션 분리(slack-format.md v2.0)의 전제이므로 누락 금지.
