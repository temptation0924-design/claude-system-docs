...
| 로컬 경로 | `/Users/ihyeon-u/haemilsia-bot` |
| Python 버전 | 3.13.2 (`runtime.txt` 파일로 지정) |
| 주요 파일 | `main.py`, `agents.py`, `news_agent.py`, `requirements.txt` |
...
수정된 파일 목록 확인:
```bash
cd /Users/ihyeon-u/haemilsia-bot
git status
git diff
```
...
5. **Python 버전** — `runtime.txt` 파일이 3.13.2로 지정되어 있는가?
...
| 빌드 실패 | Python 버전 불일치 | `runtime.txt` 확인 |
