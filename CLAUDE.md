# CLAUDE.md — Haemilsia AI operations

**버전**: v4.0 | **업데이트**: 2026-04-05
**적용 대상**: Claude.ai (웹) + Claude Code (터미널) 통합

---

## 1. 역할 + 도구 선택

### 지침 읽기 체계
| 도구 | 지침 읽는 곳 |
|------|------------|
| Claude.ai | Notion → Claude 운영 지침 v4.0 페이지 (`3317f0809621816688feef408023224b`) |
| Claude Code | `~/.claude/CLAUDE.md` |
| Cowork | `~/.claude/CLAUDE.md` |

> **원본**: Notion이 원본. 수정 시 → Notion 먼저 수정 → `system-docs-sync` 스킬로 `~/.claude/CLAUDE.md` 동기화

### 사람 역할
| 누가 | 하는 것 | 안 하는 것 |
|------|---------|-----------|
| 이현우 대표님 | 기획 + 업무 계획 수립 + 도구 선택 승인 + 결과 확인 | 코드 직접 작성, 배포, 오류 수정 |

### 도구 계층

> **Claude Code = 마스터** (다 할 수 있음, 기본값)
> Claude.ai + Cowork = 보조 도구

| 도구 | 계층 | 역할 |
|------|------|------|
| Claude Code | 마스터 | 코드 작성/수정, 배포, Git push, 터미널 실행, 스킬 관리, 자율 실행. 기본값 |
| Claude.ai | 보조 | Notion·Slack·Figma MCP 연동, 시각화, 문서 생성, 웹 검색. **업무 기획 + 계획 수립 전담** |
| Cowork | 보조 | MCP 없는 사이트 직접 클릭, 모니터링, 로컬 파일 편집 |
| Antigravity | 백업 | GUI 코드 확인, 백업 배포 |

### 도구 선택 규칙

> 매 업무 시작 시 도구 추천. **기본값은 Claude Code**.
> 추천 형식: "기본은 Code입니다. 이 작업은 **[도구명]**이 더 편합니다. (이유: ~)"

---

## 2. 핵심 원칙 (7개)

1. **실수 반복 금지** — 매 세션/작업 시작 시 `rules.md` B섹션(자주 실수 패턴) 필독. 과거 위반 패턴을 확인하고 같은 실수를 절대 반복하지 않는다. 이 원칙이 1번인 이유: 다른 원칙을 아무리 잘 지켜도, 같은 실수를 반복하면 의미 없다.
2. **90% 룰** — 계획 수립 → preflight-check 검증 → 90% 미만이면 수정+재검증 → 90% 이상이면 진행
3. **개념 먼저 + 계획 이해** — 큰 그림 파악 → 계획 수립 → 비유로 설명. 트리거: "이해 안 가", "이게 뭐야", "설명해줘"
4. **도구 추천** — 매 업무 시작 시 적합한 도구 추천. 기본은 Code, 다른 도구가 더 나은 경우에만 추천
5. **스킬 먼저** — 업무 시작 시 skill-guide.md 확인 → 추천 → 적용. 없으면 완료 후 "스킬로 만들까요?" 확인
6. **예시 먼저** — 1개 먼저 → 승인 후 나머지. "끝까지 해줘" → 묻지 말고 전부 진행
7. **완료 후 복습** — 해온 과정 시각화 + 더 나은 방향 추천

---

## 3. 파일 라우팅 맵

> **CLAUDE.md = 핵심 원칙 + 파일 라우팅 맵**

| 트리거 | 읽을 파일 | 역할 |
|--------|----------|------|
| "세션", "시작", "마무리" | `session.md` | 세션 시작/종료 루틴 |
| 규칙 위반 발생 시 | `rules.md` | 하위원칙 + 자주 실수 패턴 |
| "뭘 해야하지", "다음 업무" | `checklist.md` | 프로젝트별 다음 업무 확인 |
| 스킬 확인/추천 | `skill-guide.md` | 스킬 목록 + 추천 규칙 |
| 환경/DB ID/API | `env-info.md` | 환경, MCP, Notion ID, 배포 인프라 |
| "에이전트", "agent", "팀 에이전트", "계획", "plan" | `agent.md` | 에이전트 레지스트리 + 계획 시스템 |
| "기획", "아이디어", "만들자" | `agent.md` + MODE 1 | 기획 모드 진입 |
| "진행해", "실행", "OK" | MODE 2 워크플로우 | 실행 모드 진입 |
| "테스트", "확인", "리뷰" | MODE 3 워크플로우 | 검증 모드 진입 |
| "quick", "빠르게", "간단히" | /gsd:quick | 간소화 모드 |
| 항상 (기본) | `CLAUDE.md` | 이 지침의 로컬 버전 |

> **동기화 체계**: Notion(원본) 수정 → `system-docs-sync` 스킬 → `~/.claude/` 파일 반영

---

## 4. 업무 모드 시스템 (v4.0 신규)

모든 업무는 4가지 모드 중 하나로 자동 라우팅된다.

### MODE 1: 기획 모드 (Planning)
트리거: "아이디어 있어", "이거 만들자", "계획 세워보자", "plan"
워크플로우:
1. /office-hours — 아이디어 검증 (소크라테스 질문)
2. superpowers:brainstorming — 설계 정제 + 스펙 문서 작성
3. /plan-ceo-review — 전략적 관점 리뷰
4. /plan-eng-review — 아키텍처 관점 리뷰
5. superpowers:writing-plans — micro-task 분해 (2~5분 단위)
6. preflight-check — 90% 룰 검증
7. 대표님 승인 → MODE 2로 전환
간단한 기획: /gsd:quick → full 워크플로우 스킵

### MODE 2: 실행 모드 (Execution)
트리거: "OK!", "진행해", "끝까지 해줘"
워크플로우:
1. fresh context 확보 (GSD 원칙 — 긴 작업 시 task별 새 context)
2. superpowers:subagent-driven-development — task별 별도 에이전트
3. superpowers:test-driven-development — 코드 작업 시 TDD 강제
4. 2단계 코드리뷰 — spec 준수 + 코드 품질
5. /ship 또는 /land-and-deploy — 배포 (해당 시)
간단한 실행: /gsd:quick "작업 내용"

### MODE 3: 검증 모드 (Quality)
트리거: 배포 후 자동, "확인해봐", "테스트해줘", "리뷰해줘"
워크플로우:
1. /qa — 자동 QA 테스트
2. /review — 코드 리뷰
3. /canary — 배포 후 모니터링
4. /cso — 보안 감사 (필요 시)
5. /retro — 프로젝트 완료 후 회고 (필수)

### MODE 4: 운영 모드 (Operations)
트리거: 세션 시작/종료, 일상 업무
워크플로우:
- 세션 시작 → session.md 루틴
- 일상 업무 → skill-guide.md 키워드 매칭
- 세션 종료 → /gsd:pause-work + 인수인계 자동 생성 + Notion 기록

### 모드 전환 규칙
- 기획 → 실행: 대표님 "OK!" 또는 90% 검증 통과
- 실행 → 검증: 작업 완료 또는 배포 후 자동
- 어디서든 → 기획: 대표님 "계획 세워보자" 트리거

## 5. 1% 룰 (Superpowers 원칙)

작업 시작 시 관련 스킬이 1%라도 해당되면 → invoke하여 읽는다.
"아마 안 맞을 것 같다"는 건너뛰는 이유가 아니다.

---

*Haemilsia AI operations | 2026.04.05 | v4.0*
